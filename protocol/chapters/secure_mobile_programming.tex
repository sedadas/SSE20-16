...
%Die Dokumentation zu den jeweiligen Demoapps ist dem Abgabedokument hinzuzufügen. Dabei sollten zumindest folgende Punkte behandelt werden:
% - Beschreiben Sie detailliert die implementierte Schwachstelle der Applikation mit zumindest folgenden Punkten:
% 	-- Um welche Art von Schwachstelle handelt es sich? (kurzer theoretischer Hintergrund)
%	-- Genaue Angabe wo sich der Fehler befindet (Patch-File!)
...
% - Schritte, die erforderlich sind, um die Schwachstelle auszunutzen. Diese Angabe ist auch erforderlich, falls Sie ein Script erstellt haben.
% - Beschreibung Ihrer korrigierten App (inklusive Erläuterung, wieso diese App sicher ist).
